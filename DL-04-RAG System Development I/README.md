...
Build a complete Retrieval-Augmented Generation (RAG) system in Python, from loading a knowledge base and splitting text into chunks to retrieving relevant information and generating answers with an LLM.
...
│   │                                       #   change this - must re-run build_index.py
│   └── golden_set.json                     # Evaluation set — 54 items x 4 query variants
│
├── outputs/                                # Generated by each lab (gitignored)
...
│   ├── embedding_model.py                  # Embedding model (cached)
